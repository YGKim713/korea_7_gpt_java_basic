...
public class UserServiceImpl implements UserService {
    ...
    public void login(String userId, String password) {
        // 전달 받은 userId 에 대한 검증
        userRepository.findByUserId(userId).ifPresentOrElse(user -> {
            // 해당 userId 의 회원 정보에서 전달 받은 password 가 일치하는지 확인
            if (user.getPassword().equals(password)) {
                loggedInUser = user;
                System.out.println("로그인 성공: " + user.getName());               // ID, password 다 맞으면 나옴
            } else {
                System.out.println("로그인 실패: 비밀번호가 일치하지 않습니다.");   // password 틀리면 나옴
            }
        }, () -> System.out.println("로그인 실패: 사용자 ID를 찾을 수 없습니다.")); // ID 틀리면 나옴
    }
