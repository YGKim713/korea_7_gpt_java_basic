...
class CreditCardPaymentProcessor implements PaymentProcessor {
    ...
    public void processPayment(Order order) {
        System.out.println("신용카드로 결재를 진행합니다: " + order.getOrderId());
    }
}
...
class OrderService {
    ...
    private final PaymentProcessor paymentProcessor;        // final: 초기화 필수 => 생성자를 통해 반드시 초기화 됨
    ...
    public OrderService(PaymentProcessor paymentProcessor, GenerateReport generateReport) {
        this.paymentProcessor = paymentProcessor; // 결재 처리기 초기화
        this.generateReport = generateReport; // 보고서 생성기 초기화
    }

    // 주문 처리 메서드
    void placeOrder(Order order) {
        paymentProcessor.processPayment(order); // 걸제 처리기에 결제 로직 호출
