...
class ReportManager { // 보고서 관련 업무 담당 클래스
    ...
    // 이메일 전송을 담당
    void send() {           // 보고서 관련 클래스에 보고서와 관련 없는 이메일 작성 메서드가 들어가있음
                            // 이메일은 보고서 뿐만이 아닌 다른 곳에서도 사용됨
                            // >> SRP 원칙에 어긋남
...
class EmailSender {         // 이메일 전송 관련 메서드가 따로 분리됨
    void sendEmail() {
        System.out.println("이메일이 전송됨.");
    }
}
// 하나의 클래스에 하나의 메서드만 존재해야한다? -> X
// 보고서 클래스에 보고서 생성, 삭제, 수정 등의 관련된 메서드 생성 가능!
...
class Discount {
    double calculateDiscount(boolean isStudent, int amount) {
        if (isStudent) {            // 학생 할인을 위해 조건 추가 (코드 변경) ==> 나쁜 예!!
            return amount * 0.2;
        } else {
            return amount * 0.1;
        }
