...
// >> 좋은 소프트웨어 일수록 "모듈의 독립성"이 높다
// >> 모듈의 독립성을 결합도(Coupling)와 응집도(Cohesion)의 기준 단계로 측정

// 1. 결합도 (Coupling) - 낮은 결합도(결합도는 낮춰야함)
...
// 2. 응집도 (Cohesion) - 높은 응집도(응집도는 높여야함)
...
// >> 유지보수가 쉽고, 확장성이 좋음
// >> OOP의 5가지 원칙을 지키면, 낮은 결합도와 높은 응집도가 자연스럽게 충족됨
...
//  CreditCardPayment creditCardPayment = new CreditCardPayment();   - 클래스가 직접 객체를 생성  => X
...
public class D_Coupling_Cohesion {
    public static void main(String[] args) {
        ...
        OrderClass order = new OrderClass(payment);     // 외부에서 객체를 주입받음
