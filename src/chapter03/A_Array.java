...
// 배열 (Array)
// : "같은 타입"의 여러 변수를 하나의 집합으로 묶어서 처리하는 자료 구조
// - 데이터를 연속적으로 저장
...
public class A_Array {
    public static void main(String[] args) {
        ...
        System.out.println(array);  // 주소값 [I@43a25848 이 출력
        ...
        // System.out.println(array[3]); // ArrayIndexOutOfBoundsException 배열의 정해져있는 길이에 벗어난 접근은 허용X
        ...
        // for 문에 original.length를 직접 넣을 경우
        // > 반복할 때마다 계산을 따로 해야하기 때문에 메모리를 많이 잡아먹으므로 변수를 따로 지정하여 과부화 방지
        for (int index = 0; index < originalLength; index++) {   // 조건문/반복문에 쓰는 변수 i의 유래 : index
            if (index == removeIndex) {    // original.length - 1 = 배열 마지막 요소의 인덱스 번호 >> index < original.length 를 이용하면 마지막 index까지 반복할 수 있게 됨
        ...
        for (int number: arrayNumber) {   // 변수 number: for문 내에서만 유효한 변수
        ...
        double average = (double) total / scores.length; (double average = (double) total / length;)
         */
