...
//  }
    ...
    Cat() {
        ...
        //      super() 부모 클래스 생성자 호출 키워드가 정의되어 있음 (생략됨)
    ...
    Cat(String name) {
        ...
        super(name); // super(name, nickname); 처럼 임의로 지정 불가
                     // 부모의 생성자(Mammal(String name))에 name을 전달

        // this.name = name;  // - 주석 처리 시, 자식에게는 name 전달 X
...
public class Inheritance02 {
    public static void main(String[] args) {
        ...
        Cat cat2 = new Cat("choco");  // 자식 클래스가 생성되었습니다.
        System.out.println(cat2.name);  // Child
                        // Cat(String name)에 this.name=name; 이 문장이 있을 경우, choco로 출력됨
                        // 즉, super로 부모에도 전달되고, this로 자식에게도 전달이 됨
