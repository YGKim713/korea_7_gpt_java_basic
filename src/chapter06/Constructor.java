...
class Example1 {
    ...
    // 컴파일러가 제공하는 기본 생성자(매개변수 X, 아무런 구현X)가 생략
}
...
class Car {
    ...
    // 사용자 정의 생성자
    // 이는 기본 생성자를 오버로딩 시킨 것으로 볼 수 있다
    // 다만! 오버로딩을 시킬 때, 기본 생성자는 무조건 명시해야 함(생략 불가)
    ...
    Car(String color) {
        ...
        //      : 정의 생성자에서 다른 생성자 호출 시 반드시!! "첫 줄"에서만 호출 가능 !!

        //      - 호출된 생성자가 변수값을 초기화하는 경우
        //          해당 생성자가 수행하는 작업을 유지하기 위함(무의미함 방지!)
        this(color, "auto", 4); // 위의 생성자를 오버로딩하여 color만 입력 시 나머지 두개는 고정값으로 가져감
        // this 문장이 첫 줄이 아닐 경우 >> Error - Call to 'this()' must be first statement in constructor body
        // : 객체 생성 시 색상만 작성하는 경우 - 색상 + auto + 문 5개

        door = 5; // 변수에 재할당 > this 사용 X (생략된 것이 아님)
...
public class Constructor {
    public static void main(String[] args) {
        ...
        // Example2 example2 = new Example2();  - 빈 생성자 호출 불가
        // 인스턴스 생성 시 반드시 인자값이 필요
