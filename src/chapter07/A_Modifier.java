...
// 2. 접근 제어자 범위
// public > protected > (default) > private (제일 엄격)
...
// 2) (default) - 명시하지 않음(명시하면 Error 발생). 기본값으로 생략되어있다.
// : 같은 패키지 내에서 접근 가능 O
// : 패키지 외부에서는 접근 불가능
class DefaultClass {
    int myField;
...
// 3) protected
// : 동일한 패키지 내의 모든 클래스 내에서 접근 가능 ( = default)
// : '다른 패키지'에 있는 경우, 해당 클래스를 '상속받은 하위 클래스에서만' 접근 가능
//      즉, protected가 있는 클래스를 상속받는다면 접근 가능!
class ProtectedClass extends OtherClass {
    ...
    ProtectedClass() {
        System.out.println(super.otherField); // 상속받은 부모 클래스의 필드
...
public class A_Modifier {
    ...
    public static void main(String[] args) {
        ...
        // privateClass.myMethod; - Error
        // >> private 설정 된 필드와 메서드는
