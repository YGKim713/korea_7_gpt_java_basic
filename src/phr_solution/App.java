...
public class App {
    ...
    // main 메서드 실행 이전에 App 클래스에 존재해야하는 필드
    ...
    private static boolean processChoice(int choice) {
        switch (choice) {
            ...
            case 8: { // 건강 기록 조회 (필터링 - 진단명으로 조회): 진단명
                ...
                List<RecordResponseDto> filteredRecords = recordController.filterRecordByDiagnosis(diagnosisFilter);
    ...
    // 모든 입력값 처리를 위한 메서드 (String 반환, int 값 반환 원할 시 Integer.parseInt() 사용)
    private static String getInput(String prompt) {
        System.out.println(prompt);
        return scanner.nextLine().trim(); // trim() : 앞 뒤 공백 제거
    }

    // 각 String 타입 데이터 입력 받기 위한 메서드
