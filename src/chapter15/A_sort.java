...
*
* 2. 중간(가공) 절차
*   - map(Function)       // Function : 입력(인자, 매개변수)을 받아서 작업을 수행하고, 결과를 출력(반환값)으로 내보냄
*   - filter(Predication) // Predicate : argument를 받아 boolean으로 반환
*   - sorted(Comparator)  // Comparator : 사용자 정의 객체를 사용하거나 클래스 내에서 비교 메서드를 구현하고자 할 때,
*                                         또는 본인만의 다른 비교방식을 사용하고자 할 때 사용*
* 3. 최종(결과) 절차
*   - collect
*   - forEach
* */
...
public class A_sort {
    public static void main(String[] args) {
        ...
        // : 객체간의 비교를 실시
        // - 기본 데이터 타입에 대한 비교 X, Comparator는 '객체 타입'에 대한 비교 O ( int X // Integer O )
        ...
        List<String> sortedCharactoer = characterNames.stream()
                .sorted((n1, n2) -> n1.length() - n2.length()) // 오름차순
                .collect(Collectors.toList());

        System.out.println(sortedCharactoer); // [Tom, Jerry, Ruffy, Pororo]
