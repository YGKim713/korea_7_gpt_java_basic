...
public class BoardServiceImpl implements BoardService {
    ...
    public BoardResponseDto findById(long id) {
        // 요청: 매개변수의 id 데이터를 DB 에서 검색 후 반환
        Optional<Board> board = boardRepository.findById(id);

        // 해당 데이터를 응답 객체로 변환
        BoardResponseDto boardResponseDto = board
                .map(BoardResponseDto::fromEntity)
                // 응답 객체로 변환 시 null 값이 존재한다면 예외 발생시킴
    ...
    public void update(long id, BoardRequestDto boardRequestDto) {
        Board existingBoard = boardRepository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("게시글을 찾을 수 없습니다."));
        // orElseThrow 로 null 값을 무조건 예외처리하기 때문에 findById를 Optional 타입으로 하지 않아도 됨
    ...
    public void delete(long id) {
        if (!boardRepository.findById(id).isPresent()) {    // 해당 id의 게시글이 존재하지 않는다면 (! 연산자 확인)
