...
public class Z_Array_List {
    public static void main(String[] args) {
        ...
        // : 0 부터 시작하는 저장 순서를 가지며 해당 Index 번호를 통해 데이터를 접근
        // : 연속된 메모리 공간에 저장
        ...
        // Array {1, 2, 3, 4, 5} -> 2 삭제 -> {1, 0, 3, 4, 5} => Index는 고유값이기 때문에 바뀌지 X
        // List {1, 2, 3, 4, 5} -> 2 삭제 -> {1, 3, 4, 5} => Index는 데이터의 위치만 확인하는 값, 바뀜 O
        // : 불연속적 메모리 공간을 차지
        ...
        // immnutableList.set(0, "블루베리");   // >> 문장 자체는 오류 X, 실행 시 오류 O
        List<String> dynamicList = new ArrayList<>();
        dynamicList.add("오렌지");              // >> 가변 리스트이기 때문에, 실행 시 오류 X
        dynamicList.add("망고");
        ...
        changeList.add("딸기");
        System.out.println(changeList); // [사과, 바나나, 체리, 감, 딸기]     >> 정적 -> 동적 변환

        List<String> changeList2 = new ArrayList<>();
        changeList2 = List.of("과일1", "과일2");
        // changeList2.add("과일3"); // Error!             >> 동적 -> 정적 변환

        // +) Arrays.asList()
        //      : 배열을 리스트로 변환하는 메서드
        List<String> asList = Arrays.asList("사과", "바나나", "키위");
        // asList.add("수박");   >> Error!    Arrays.asList() 는 원소 추가 삭제 불가능
        List<String> changeList3 = new ArrayList<>(asList);
        changeList3.add("수박");
        System.out.println(changeList3); // [사과, 바나나, 키위, 수박]  >> ArrayList 로 바꾸면 원소 추가 가능
        ...
        // +) Arrays.asList()가 반환하는 리스트는 가짜 ArrayList (asList 와 new ArrayList 각각 Ctrl+클릭 으로 확인)
