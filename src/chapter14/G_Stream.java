...
public class G_Stream {
    public static void main(String[] args) {
        ...
        // 2. 중간 연산
        // : filter(조건에 맞는 요소만 선택 >> 요소 개수 변화 O), map(요소를 변환 >> 요소 개수 변화 X),
        //   sorted(요소 정렬), distinct(중복 제거),
        //   limit(처음부터 지정된 개수만큼 선택), skip(처음부터 지정된 개수만큼 건너뜀) 등

        // 3. 최종 연산
        // : forEach(각 요소를 처리 >> 스트림으로 반환 X, 소모하는 개념), collect(요소를 변환하여 컬렉션으로 반환),
        //   reduce(모든 요소를 하나의 값으로 줄임), count(요소의 개수를 반환) 등

        names.stream()
                .map(name -> name + "님")
                .filter(name -> name.startsWith("이")) // endsWith("님") >> "님"으로 끝나는 조건
             // .forEach(name -> System.out.println(name)); // 이승아님
                // (instance::instanceMethod)
                .forEach(System.out::println); // 이승아님
