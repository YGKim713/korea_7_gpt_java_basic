...
// 2. 추상 클래스 특징
// 1) 클래스 자체로 인스턴스 생성 X (필드 생성 가능)
// 2) 하나 이상의 추상 메서드를 포함 (일반 메서드 작성 가능)
// 3) 확장성 제공을 위해 사용
//      : 자식 클래스들이 공통된 기능을 공유 + 고유 기능을 구현하도록 강제
// 4) 다중 상속 불가능 ! - 하나의 extends 만 가능 ( class A extends B extends C -- 불가능 !!)
...
class Police extends Human {
    // 추상 메서드를 구현
    // >> 구현하지 않으면 다음의 Error가 발생
    // Class 'Police' must either be declared abstract or implement abstract method 'work()' in 'Human'
    void work() {
