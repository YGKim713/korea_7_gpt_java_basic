...
// - 구성요소: 추상 메서드, 일반(구현) 메서드, 필드, 생성자
// - 단일 상속만 가능 (extends_확장하다)
// >> 기본 구현을 제공하고 확장 가능
...
interface PlayingCard {
    ...
    int CLOVER = 1;             // 네가지 모두 같은 특성을 가지는 상수 (public static final)
    ...
    String getCardKind();   // 둘 다 같은 특성을 가지는 메서드 (public abstract)
    ...
    // cf) 디폴트 메서드 - interface 내부에서 반드시 default 키워드 작성  --> Java 8 버전 이후 생긴 조건
    // : 생략 시 구현부가 존재할 수 없다는 오류 발생
    // void noDefault() {} - Error - Interface abstract methods cannot have body
...
class Card implements PlayingCard {
    ...
    // >> public을 지정하지 않으면 기본 default로 지정됨 >> 오류 발생
    // getCardNumber()' in 'chapter08.Card' clashes with 'getCardNumber()' in 'chapter08.PlayingCard'; attempting to assign weaker access privileges ('package-private'); was 'public'
    ...
    //      : 인터페이스 자체가 "어떤 동작을 외부에서 사용할 수 있음"을 보장하고 있기 때문
}
