...
*        - null 값이 할당 될 수 있는 Wrapper 객체를 언박싱 할 경우, NullPointerException 발생 위험이 있음
*
*       cf) Wrapper 클래스의 System.out.println 출력
...
public class A_Wrapper {
    public static void main(String[] args) {
        ...
        // 자동 박싱: 기본 타입 int 의 값을 Integer 객체로 변환
        ...
        // 자동 언박싱: Integer 객체를 기본 타입 int 의 값으로 변환
        int num = integer;
        ...
        // - parseInt(String s): 문자열을 기본 int 타입으로 파싱(분석)
        ...
        System.out.println(result); // -1 (호출 객체인 myInteger 가 더 작음)
        int result2 = anotherInteger.compareTo(myInteger);
        System.out.println(result2); // 1 (호출 객체인 anotherInteger 가 더 큼)
        Integer myInteger2 = new Integer(100);
        int result3 = myInteger.compareTo(myInteger2);
        System.out.println(result3); // 0 (호출 객체와 비교 객체의 값이 같음)
    }
}
